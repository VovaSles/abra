...
export function fetchOptions(q) {
    return async dispatch => {
        if (q) {
            axios.get(`https://dataservice.accuweather.com/locations/v1/cities/autocomplete?apikey=${process.env.REACT_APP_API_KEY}&q=${q}`)
...
export function fetchWeather(id) {
    return dispatch => {
        try {
            ...
            //Current Conditions
            axios.get(`https://dataservice.accuweather.com/currentconditions/v1/${id}?apikey=${process.env.REACT_APP_API_KEY}`)
                .then(res => dispatch(setCityAction(res.data[0])))
                .catch(err => dispatch(setErrorAction(err)))


            // Days of Daily Forecasts
            axios.get(`https://dataservice.accuweather.com/forecasts/v1/daily/5day/${id}?apikey=${process.env.REACT_APP_API_KEY}`)
